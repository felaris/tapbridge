...
REPO="felaris/tapbridge"

# Detect architecture now
ARCH=$(uname -m)
if [ "$ARCH" = "arm64" ]; then
  FILE="tapbridge-mac-arm64"
else
  FILE="tapbridge-mac-intel"
fi
...
DEST="$HOME/.local/bin/tapbridge"

# Ensure destination directory exists
mkdir -p "$HOME/.local/bin"

echo "Downloading TapBridge $TAG ($ARCH)..."
